...
def main():
    ...
    mass=float(input("Enter the mass of a chemical: "))
    periodic_table=make_periodic_table()

    for element in periodic_table:
        name, atomic_mass =element
        print(f"{name}: {atomic_mass} g/mol")
    
    symbol_quantity_list = parse_formula(formula, periodic_table)
    mass = compute_molar_mass(symbol_quantity_list, periodic_table)
    # H2number_of_moles = sample_mass / mass
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for symbol, quantity in symbol_quantity_list:
        atomic_mass = periodic_table_dict[quantity]
